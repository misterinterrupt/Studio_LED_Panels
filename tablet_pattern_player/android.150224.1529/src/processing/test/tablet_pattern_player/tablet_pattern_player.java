...
int buttonGridXoffset = appPaddingWidth + patternPreviewWidth;
...
int[][] patternButton_xyPos = new int[][] [numPatternButtons][];  // pattern button draw positions, array of [x,y]
...
public void setup() {
  ...
  for(int j=0; j< numPatternButtonRows; j++) {
    for (int i=0; i < numPatternButtonCols; i++) {
      ...
      patternButton_xyPos[i][1] = buttonGridYoffset + (ui_yposMultiplier * i);
    }
  }
  ...
  for(int b=0; b<numPatternButtons; b++) {
    ...
    patternButtons[b] = cp5.addButton("seq" + (b+1))
      ...
      .setValue(0)
      .setPosition(patternButton_xyPos[b][1], patternButton_yPos[b][0])
